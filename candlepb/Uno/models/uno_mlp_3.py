...
def create_mlp_block(cell, input_node, skipco_inputs):
        ...
        previous_node = mergeNode1
        for i in range(8):
            n_i = VariableNode(f'N{i+3}')
            create_mlp_node(n_i)
            block.add_node(n_i)

            block.add_edge(previous_node, n_i)

            mergeNode_i = VariableNode(name='Merge')
            mergeNode_i.add_op(AddByPadding())
            mergeNode_i.add_op(Concatenate())
            block.add_node(mergeNode_i)

            block.add_edge(n_i, mergeNode_i)
            #block.add_edge(mergeNode1, mergeNode_i) # residual connection
            # replaced by skipco
            cnode = get_skipco_node()
            for inpt in skipco_inputs:
                cnode.add_op(Connect(cell.graph, inpt, cnode))
            block.add_node(cnode)
            block.add_edge(cnode, mergeNode_i)

            # preparing next iter
            skipco_inputs.append(n2)
            skipco_inputs.append(mergeNode_i)
            previous_node = mergeNode_i
...
def create_structure(input_shape=[(2,) for _ in range(4)], output_shape=(1,), num_cell=8, *args, **kwargs):
    ...
    create_mlp_block(cell_last, pred_cell.output, skipco_inputs)
